...
router.post('/', async (req, res) => {
 ...
 try {
  ...
  const productCreateData = await Product.create(req.body)
    .then((product) => {
    ...
    res.status(200).json(productCreateData);
  }catch (err)  {
      console.log(err);
...
router.put('/:id', async (req, res) => {
 ...
 const productUpdateData = await Product.update(req.body, {
    where: {
      id: req.params.id,
    },
  })
    ...
    res.status(200).json(productUpdateData);
